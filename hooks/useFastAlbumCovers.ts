...
export function useFastAlbumCovers({
  ...
  const loadCover = useCallback(async (song: Song) => {
    ...
    try {
      ...
      // First check if song already has a predefined album cover
      if (song.albumCover && song.albumCover.trim() !== '') {
        cover = song.albumCover;
      } else {
        // Otherwise, search iTunes API
        cover = await fastItunesService.getAlbumCover(song.artist, song.title);
      }
